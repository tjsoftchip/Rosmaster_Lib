...
class Rosmaster:
    ...
    def set_akm_steering_angle(self, angle, ctrl_car=False):
        ...
        # 调用原有逻辑（STM32 控制舵机）
        try:
            if int(angle) > 22 or int(angle) < -22:
                return
    ...
    def set_car_motion(self, v_x, v_y, v_z):
        ...
        R2/R2L: v_x=[-1.8, 1.8], v_y=[-0.045, 0.045], v_z=[-1, 1]
        R2_MSSD: v_x=[-1.8, 1.8], v_y=[-0.045, 0.045], v_z=[-1, 1] (大车底盘)

        Args:
            v_x: 线速度（m/s）
            v_y: 侧向速度（m/s，R2 车型忽略）
            v_z: 归一化转向控制值（-1~1 映射到 -22°~22°）
        """
        if self.is_r2_mssd:
            # R2_MSSD 模式（大车底盘）
            # 
            ...
            # 2. 转向角度计算（直接线性映射）
            # ============================================================
            # v_z 范围 -1 ~ 1，直接映射到转向角度 -22° ~ 22°
            # 机械限制：最大转向角度 ±22°（实际测量值）
            # ============================================================

            MAX_STEER_ANGLE = 22  # 最大转向角度（度），机械极限
